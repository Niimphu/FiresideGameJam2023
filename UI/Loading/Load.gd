...
onready var load_scene = preload("res://UI/Loading/LoadingScreen.tscn")
...
func change_scene(current_scene, new_scene):
	...
	yield(get_tree().create_timer(0.5), "timeout") #delay
	...
	while true:
		...
		if error == OK:
			...
			yield(get_tree().create_timer(1), "timeout") #delay
